...
class MysqlEntity
{
    ...
    */
    ...
    {
        $query = 'DROP TABLE IF EXISTS `'.get_class($this)::TABLE_NAME.'`;';
    ...
    */
    ...
    {
        $query = 'TRUNCATE TABLE `'.get_class($this)::TABLE_NAME.'`;';
        if($this->debug)echo '<hr>'.get_class($this).' ('.__METHOD__ .') : Requete --> '.$query.'<br>'.$this->dbconnector->connection->error;
        $myQuery = $this->customQuery($query);
    }
    ...
    */
    public function create($debug=false){
        $query = 'CREATE TABLE IF NOT EXISTS `'.get_class($this)::TABLE_NAME.'` (';
    ...
    public function massiveInsert($events){
        ...
        $query = 'INSERT INTO `'.get_class($this)::TABLE_NAME.'`(';
            $i=false;
            foreach($this->object_fields as $field=>$type){
                if($type!='key'){
                    if($i){$query .=',';}else{$i=true;}
    ...
    */
    public function save($id_field='id'){
        if(isset($this->$id_field)){
            $query = 'UPDATE `'.get_class($this)::TABLE_NAME.'`';
            $query .= ' SET ';
    ...
    */
    public function change($columns,$columns2,$operation='=',$debug=false){
        $query = 'UPDATE `'.get_class($this)::TABLE_NAME.'` SET ';
        $i=false;
    ...
    */
    public function loadAll($columns,$order=null,$limit=null,$operation="=",$debug=false,$selColumn='*'){
        ...
        $whereClause = $this->getWhereClause($columns,$operation);
            ...
            $query = 'SELECT '.$selColumn.' FROM `'.get_class($this)::TABLE_NAME.'` '.$whereClause.' ';
    ...
    */
    ...
    protected function queryFilter($query) {
        ...
        if (strpos($query,'##FIRST_USER_LOGIN##')) {
            $firstUserLogin = $this->dbconnector->connection->query('SELECT login FROM '.User::TABLE_NAME.' ORDER BY id LIMIT 1;');
