...
\n";
    ...
    case 'removeFolder':
        ...
        if(isset($_['id']) && is_numeric($_['id']) && $_['id']>0){
            $eventManager->customQuery('DELETE FROM `'.Event::TABLE_NAME.'` WHERE `'.Event::TABLE_NAME.'`.`feed` in (SELECT `'.Feed::TABLE_NAME.'`.`id` FROM `'.Feed::TABLE_NAME.'` WHERE `'.Feed::TABLE_NAME.'`.`folder` =\''.intval($_['id']).'\') ;');
