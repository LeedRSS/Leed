			...
			<aside>
				...
				<ul class="clear">
						...
						<?php $counter1=-1; if( isset($folders) && is_array($folders) && sizeof($folders) ) foreach( $folders as $key1 => $value1 ){ $counter1++; ?>
							...
							<?php $unreadEventsForFolder=$this->var['unreadEventsForFolder']="";?>
						...
						<ul <?php if( !$value1->getIsopen() ){ ?>style="display:none;"<?php } ?>>
								...
								<?php if( count($feeds)!=0 ){ ?>
									...
									<?php $counter2=-1; if( isset($feeds) && is_array($feeds) && sizeof($feeds) ) foreach( $feeds as $key2 => $value2 ){ $counter2++; ?>
										...
										<li> 
											<a href="index.php?action=selectedFeed&feed=<?php echo $value2['id'];?>" alt="<?php echo $value2['url'];?>" title="<?php echo $value2['url'];?>"><?php echo $value2['name'];?> </a>
					...
					<section class="<?php if( !$value1->getUnread() ){ ?> eventRead <?php } ?> <?php echo $hightlighted%2==0?'eventHightLighted':'';?>" >
						...
						<h3 class="articleDetails">
							<?php if( $articleDisplayLink ){ ?>
								...
								<a href="<?php echo $value1->getLink();?>" target="_blank"><?php echo $allFeeds['idMap'][$value1->getFeed()]['name'];?></a>
							<?php } ?>
				...
				<?php if( $pages!=0 ){ ?>
					...
					<p>Page <?php echo $page;?>/<?php echo $pages;?> : 
						...
						<?php $counter1=-1; if( isset($pagesArray) && is_array($pagesArray) && sizeof($pagesArray) ) foreach( $pagesArray as $key1 => $value1 ){ $counter1++; ?>
							...
							<?php if( $action=='selectedFeed' ){ ?> 
								&feed=<?php echo $currentFeed->getId();?> 
							<?php } ?> 
							<?php if( $action=='selectedFolder' ){ ?> 
								&folder=<?php echo $currentFolder->getId();?>

							<?php } ?>

								&page=<?php echo $value1;?>"><?php echo $value1;?>
