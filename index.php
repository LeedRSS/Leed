		...
		<div id="main" class="wrapper clearfix">
			...
			<aside>
				...
				<ul class="clear">
					...
					?>
					<!-- DOSSIER -->
					<li><h1 class="folder"><a href="index.php?action=selectedFolder&folder=<?php echo $folder->getId(); ?>"><?php  echo $folder->getName();?></a><?php if($unreadEventsForFolder!=0){ ?> <a class="readFolder" title="Parcourir les non lus du dossier" alt="Parcourir les non lus du dossier" <?php if(count($feeds)!=0){ ?>onclick="toggleFolder(this,<?php echo $folder->getId(); ?>);"<?php } ?> >Deplier</a> <a class="unreadForFolder" alt="marquer comme lu le(s) <?php echo $unreadEventsForFolder; ?> evenement(s) non lu(s) de ce dossier" title="marquer comme lu le(s) <?php echo $unreadEventsForFolder; ?> evenement(s) non lu(s) de ce dossier" onclick="if(confirm('Tout marquer comme lu pour ce dossier?'))window.location='action.php?action=readFolder&folder=<?php  echo $folder->getId(); ?>';"><?php echo $unreadEventsForFolder.' non lu</a>'; } ?></h1>
						<!-- FLUX DU DOSSIER -->
